...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      let res: { success: boolean; user?: { role: string } };
      if (mode === "signup") {
        res = await api.register({ name, email, password });
      } else {
        res = await api.login({ email, password, role: loginRole });
      }
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {mode === "login" && (
            <div className="space-y-2">
              ...
              <p className="text-xs text-gray-500">Select User or Admin. Access is denied if role does not match.</p>
            </div>
          )}
          {error && (
